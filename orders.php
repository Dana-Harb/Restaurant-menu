  ...
  <body>
              ...
              <ul class="nav navbar-nav navbar-right ml-auto">	
                ...
                if(isset($_SESSION['user'])){
                  ...
                  <div class="dropdown-menu">
                    ...
                    <a class="dropdown-item" href="orders.php?logout=1">Logout</a>
                  </div>
              </li>';
              echo $s;
    ...
    <section class="blog-posts grid-system">
      ...
      if(isset($_SESSION['user'])){
        $mysqli = new mysqli('localhost','root','','restaurant');
        if($mysqli->connect_error) exit('error connecting to db');
        $sql = "SELECT * FROM ordertb where CUSTID=$_SESSION[user] ORDER BY orderdate DESC";
        $stmt = $mysqli->prepare($sql);
        $stmt->execute();
        ...
        echo '<table class="orders" >';
          foreach($orders as $key){
             $s='<tr>';
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order number: </span>#'.$key['ORDERID'].'</td></tr>';
            $s.='<tr>';
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Date: </span>'.$key['orderdate'].'</td></tr>';
            $s.='<tr>';
            if($key['STATUS']== -1){
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Status: </span><span style="color: red">Rejected</span></td></tr>';
            }else if($key['STATUS']== 0){
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Status: </span><span style="color: black">Pending</span></td></tr>';
            }else if($key['STATUS']== 1){
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Status: </span><span style="color: orange">Accepted</span></td></tr>';
            }else if($key['STATUS']== 2){
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Status: </span><span style="color: goldenrod">Being Prepared</span></td></tr>';
            }else if($key['STATUS']== 3){
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Status: </span><span style="color: darkmagenta">Shipped</span></td></tr>';
            }else if($key['STATUS']== 4){
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Status: </span><span style="color: green">Recieved</span></td></tr>';
            }
            $sql = "SELECT * FROM itemorder as iorder join item as i on iorder.ITEMID=I.ITEMID where ORDERID=$key[ORDERID]";
            $stmt = $mysqli->prepare($sql);
            $stmt->execute();
            $items = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);
            foreach($items as $item){
              $s.='<tr>';
                $s.='<td style="font-weight:bold">'.$item['ITEMNAME'].' X'.$item['COUNT'].'</td>';
                $s.='<td>'.number_format((float)$item['PRICE']*$item['COUNT'], 2, '.', ',').' LBP</td></tr>';
            }
            $s.='<tr style="border-bottom:1px dashed #fd7e14">';
            $s.='<td style="text-align:right; font-weight:bold">TOTAL:  </td>';
            $s.='<td>'.number_format((float)$key['TOTAL'], 2, '.', ',').' LBP</td></tr>';
              echo $s;
          }
        echo '</table></div>';
        ...
        <div class="row">
          <div class="col-lg-12">
            <ul class="social-icons">
              <li><a href="#">Ali Isamil 92259</a></li>
              <li><a href="#">Sarah Zaidan 98989</a></li>
              <li><a href="#">Dana Harb 94672</a></li>
            </ul>
          </div>
        </div>
      </div>
