...
if(isset($_POST['delete'])){
	...
	mysqli_query($conn,"DELETE FROM `admin` WHERE ADID='$_POST[delete]'");
}
...
<body>
            ...
            <li class="nav-item">
              ...
              <ul class="nav navbar-nav navbar-right ml-auto">	
                ...
                if(isset($_SESSION['user'])){
                  ...
                  <div class="dropdown-menu">
                    ...
                    <a class="dropdown-item" href="admintable.php?logout=1">Logout</a>
                  </div>
              </li>';
              echo $s;
              ...
              echo $s;
                ...
                <ul class='dropdown-menu form-wrapper'>					
                    <li>
                        <form name='form1' method='post'  action='admintable.php'>
    ...
    <?php
        ...
        $sql = "SELECT `ADID`, `ADNAME`, `USERNAME`, `EMAIL`, `PHONE` FROM `admin`";
        $result = $conn->query($sql);

        if ($result->num_rows > 0) {
            echo "<table class='styled-table'><thead><tr><th>ID</th><th>Name</th><th>Username</th><th>Email</th><th>Phone</th><th>Delete</th></tr></thead>";
            // output data of each row
            while($row = $result->fetch_assoc()) {
                echo "<tr><td>" . $row["ADID"]. "</td><td>" . $row["ADNAME"]. "</td><td>" . $row["USERNAME"]. "</td><td>" . $row["EMAIL"]. "</td><td>";
                 if(strlen($row["PHONE"])==7){
                     $row["PHONE"]="0".$row["PHONE"];
                }
                echo $row["PHONE"]. "</td><td>";
                echo"<form method='post' action='admintable.php' >";
                echo'<button type="submit" name="delete" value="'.$row["ADID"].'">Delete</button>';
                echo "</form>";
                echo "</td></tr>";
