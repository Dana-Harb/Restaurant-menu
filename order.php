...
if(isset($_POST['change'])){
  ...
  foreach($_POST['result'] as $key){
    $sql = "UPDATE `ordertb` SET `STATUS`= '$_POST[$key]' WHERE `ORDERID`= $key ";  
  ...
  <body>
    ...
    <section class="blog-posts grid-system">
      <?php
        $mysqli = new mysqli('localhost','root','','restaurant');
        if($mysqli->connect_error) exit('error connecting to db');
        $sql = "SELECT * FROM ordertb as o join customer as c on o.CUSTID=c.CUSTID";
        $stmt = $mysqli->prepare($sql);
        $stmt->execute();
        ...
        $ary = array();
          foreach($orders as $key){
            if(($key['STATUS']!= 4 && $key['STATUS']!= -1)){
             ...
             $s='<tr>';
              ...
              $s.='<tr>';
                $s.='<td colspan="2"><span style="color: #7a7a7a">Address: </span>'.$key['ADDRESS'].'</td></tr>';
              $s.='<tr>';
              $s.='<td colspan="2"><span style="color: #7a7a7a">Order Status: </span><select name="'.$key['ORDERID'].'">
                <option value="-1"';if ($key['STATUS'] == -1)$s.="selected";$s.='>rejected</option>
                <option value="0"';if ($key['STATUS']  == 0) $s.="selected";$s.='>Pending</option>
                <option value="1"';if ($key['STATUS']  == 1) $s.="selected";$s.='>Accepted</option>
                <option value="2"';if ($key['STATUS']  == 2) $s.="selected";$s.='>Being Prepared</option>
                <option value="3"';if ($key['STATUS']  == 3) $s.="selected";$s.='>Shipped</option>
                <option value="4"';if ($key['STATUS']  == 4) $s.="selected";$s.='>Received</option>
                </select></td></tr>';
          ...
          echo '<tr><td></td><td> <button name="change" class="button5">Change</button> </td>'; 
